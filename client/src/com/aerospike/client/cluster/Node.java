 ...
 * 
 * Portions may be licensed to Aerospike, Inc. under one or more contributor license agreements
 * WHICH ARE COMPATIBLE WITH THE APACHE LICENSE, VERSION 2.0.
 * 
...
public class Node implements Closeable {
  ...
  protected volatile RateLimiter qPSRateLimiter;
  ...
  public Node(Cluster cluster, NodeValidator nv) {
    ...
    active = true;
    if (cluster.maxQPSperNodeForClient != null) {
      qPSRateLimiter = RateLimiter.create(cluster.maxQPSperNodeForClient.doubleValue());
      Log.info(String.format("RateLimiter Enabled for this client. Per Node Per Client QPS permitted - %f",qPSRateLimiter.getRate()));
    }
      
  }

  public boolean tryAcquirePermit() {
    return qPSRateLimiter != null ? qPSRateLimiter.tryAcquire() : true;
  }
  ...
  public final void refresh(Peers peers) {
    ...
    try {
      if (tendConnection.isClosed()) {
        tendConnection =
            new Connection(cluster.tlsPolicy, host.tlsName, address,
                cluster.getConnectionTimeout(), cluster.maxSocketIdleMillis);
      }

      if (peers.usePeers) {
        HashMap<String, String> infoMap =
            Info.request(tendConnection, "node", "peers-generation", "partition-generation");
        verifyNodeName(infoMap);
        verifyPeersGeneration(infoMap, peers);
        verifyPartitionGeneration(infoMap);
      } else {
        String[] commands =
            cluster.useServicesAlternate ? new String[] {"node", "partition-generation",
                "services-alternate"} : new String[] {"node", "partition-generation", "services"};
  ...
  private final void addFriends(HashMap<String, String> infoMap, Peers peers)
  ...
  protected final void refreshPeers(Peers peers) {
    // Do not refresh peers when node connection has already failed during this cluster tend
    // iteration.
        ...
        for (Host host : peer.hosts) {
          try {
            ...
            if (!peer.nodeName.equals(nv.name)) {
              ...
              if (Log.warnEnabled()) {
                Log.warn("Peer node " + peer.nodeName + " is different than actual node " + nv.name
                    + " for host " + host);
              }
  ...
  protected final void refreshPartitions(Peers peers) {
    // Do not refresh partitions when node connection has already failed during this cluster tend
    // iteration.
    ...
    try {
      ...
      PartitionParser parser =
          new PartitionParser(tendConnection, this, cluster.partitionMap, Node.PARTITIONS,
              cluster.requestProleReplicas);
  ...
  public final Connection getConnection(int timeoutMillis) throws AerospikeException {
    ...
    if (connectionCount.getAndIncrement() < cluster.connectionQueueSize) {
      try {
        conn =
            new Connection(cluster.tlsPolicy, host.tlsName, address, timeoutMillis,
                cluster.maxSocketIdleMillis);
      ...
      connectionCount.getAndDecrement();
      throw new AerospikeException.Connection(ResultCode.NO_MORE_CONNECTIONS, "Node " + this
          + " max connections " + cluster.connectionQueueSize + " would be exceeded.");
    }
  }
