...
public class TestOperateList extends TestSync {
	...
	@Test
	public void listBinsRoundTripNullValueSlots() {
		Key keyAppend = new Key(args.namespace, args.set, "oplAppendNullItem");
		client.delete(null, keyAppend);

		List<Value> items = new ArrayList<>();
		items.add(Value.get(5L));
		items.add(null);
		items.add(Value.get("trail"));

		Record record = client.operate(null, keyAppend, ListOperation.appendItems(binName, items));
		assertRecordFound(keyAppend, record);

		Record gotAppend = client.get(null, keyAppend, binName);
		assertRecordFound(keyAppend, gotAppend);
		List<?> listAppend = gotAppend.getList(binName);
		assertEquals(3, listAppend.size());
		assertEquals(5L, listAppend.get(0));
		assertNull(listAppend.get(1));
		assertEquals("trail", listAppend.get(2));

		Key keyInsert = new Key(args.namespace, args.set, "oplInsertNullItem");
		client.delete(null, keyInsert);
		client.operate(null, keyInsert, ListOperation.append(binName, Value.get(1)));

		List<Value> insertList = new ArrayList<>();
		insertList.add(null);
		insertList.add(Value.get(2L));
		client.operate(null, keyInsert, ListOperation.insertItems(binName, 0, insertList));

		Record gotInsert = client.get(null, keyInsert, binName);
		assertRecordFound(keyInsert, gotInsert);
		List<?> listInsert = gotInsert.getList(binName);
		assertEquals(3, listInsert.size());
		assertNull(listInsert.get(0));
		assertEquals(2L, listInsert.get(1));
		assertEquals(1L, listInsert.get(2));

		Key keyPut = new Key(args.namespace, args.set, "oplPutNullSlot");
		client.delete(null, keyPut);

		Value[] arr = new Value[] { Value.get(10L), null, Value.get("z") };
		client.put(null, keyPut, new Bin(binName, Value.get(arr)));

		Record gotPut = client.get(null, keyPut, binName);
		assertRecordFound(keyPut, gotPut);
		List<?> listPut = gotPut.getList(binName);
		assertEquals(3, listPut.size());
		assertEquals(10L, listPut.get(0));
		assertNull(listPut.get(1));
		assertEquals("z", listPut.get(2));
	}
}
