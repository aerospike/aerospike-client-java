...
public class TestIndex extends TestSync {
	...
	public void createDrop() {
		...
		for (Node node : nodes) {
			String cmd = IndexTask.buildStatusCommand(args.namespace, indexName, node.getServerVersion());
	...
	public void setIndexCreateDrop() {
		...
		for (Node node : nodes) {
			String cmd = IndexTask.buildExistsCommand(args.namespace, setIndexName, node.getServerVersion());
			String response = Info.request(node, cmd);
		...
		for (Node node : nodes) {
			String cmd = IndexTask.buildStatusCommand(args.namespace, setIndexName, node.getServerVersion());
			String response = Info.request(node, cmd);
			int code = Info.parseResultCode(response);
